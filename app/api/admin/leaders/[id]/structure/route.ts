...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const structureResult = await db.query(`
      SELECT b.id as block_id, b.title as block_title, b.order_index as block_order,
    ...
    for (const row of structureResult.rows) {
      ...
      if (!blocksMap.has(row.block_id)) {
        blocksMap.set(row.block_id, { id: row.block_id, title: row.block_title, order_index: row.block_order, modules: new Map() });
      }
